...
# Load stc
file = os.path.join(base_dir, "mean_stc")
...
for f in freqs:
   ...
   if np.all(relevant_data==0): 
      ...
      plot_stc(savedir = os.path.join(base_dir, "0315_group_coords", hemi), brain_to_plot = t_stc, t = f, center_of_mass = COM_vertex)
      COM_coord = inflated_surface_rr[COM_vertex]
      COM_list.append(COM_coord)

make_3d_plot_spatiospec(savedir = os.path.join(base_dir, "0315_group_coords"), COM_list = COM_list, name = hemi + " group centroids")

file = os.path.join(base_dir, f"mean_stc_COM_8-30_{hemi}.pkl") 
