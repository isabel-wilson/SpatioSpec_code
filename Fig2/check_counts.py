...
hemi = "lh"
cluster_of_interest = "large" # outliers-no_large
...
if hemi == "rh": 
   if cluster_of_interest == "large": 
      mask = labels == 1 # for rh, large cluster is "1"
   elif cluster_of_interest == "small": 
      mask = labels == 0 # for rh, small cluster is "0"
   elif cluster_of_interest == "outliers-no_large":
...
with open(os.path.join(base_dir, f"individual_distances_from_centroid_cluster_{cluster_of_interest}_{hemi}_normalized.pkl"), "rb") as f:
   distances = pickle.load(f)

# FILTER: Apply mask to distances and choose frequencies of interest
filtered = distances[mask]
filtered = filtered[filtered["freq"].isin(freqs)]
...
filtered_stats = (
    ...
    .agg(
        ...
        avdist_in_cluster=("distance_normed", "mean")
    )
...
distances_stats = (
    ...
    .agg(
        avdist_all=("distance_normed", "mean")
    )
    .reset_index()
)
...
# Save
stats.columns = ["subject", f"number_in_cluster_{hemi}", f"avdist_in_cluster_{hemi}", f"avdist_all_{hemi}"]
stats.to_feather(os.path.join(base_dir, f"average_distance_counts_cluster_{cluster_of_interest}_{hemi}_10-30.rds"))
